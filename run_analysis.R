...
train <- read.table("getdata_projectfiles_UCI HAR Dataset/UCI HAR Dataset/train/X_train.txt",header=FALSE)
test <- read.table("getdata_projectfiles_UCI HAR Dataset/UCI HAR Dataset/testX_test.txt",header=FALSE)
data_complete <- rbind(train, test)                               # raw data

# 4.  Appropriately labels the data set with descriptive variable names. 

feature_list <- read.table("getdata_projectfiles_UCI HAR Dataset/UCI HAR Dataset/features.txt",header=FALSE)[,2]
...
train_label <- read.table("getdata_projectfiles_UCI HAR Dataset/UCI HAR Dataset/train/Y_train.txt",header=FALSE)
test_label <- read.table("getdata_projectfiles_UCI HAR Dataset/UCI HAR Dataset/test/Y_test.txt",header=FALSE)
activity_label <- c(train_label[[1]],test_label[[1]])                  

# get activity name - string
act_table <- read.table("getdata_projectfiles_UCI HAR Dataset/UCI HAR Dataset/activity_labels.txt",header=FALSE)
...
objID_train <- read.table("getdata_projectfiles_UCI HAR Dataset/UCI HAR Dataset/train/subject_train.txt",header =FALSE)
objID_test <- read.table("getdata_projectfiles_UCI HAR Dataset/UCI HAR Dataset/test/subject_test.txt",header=FALSE)
